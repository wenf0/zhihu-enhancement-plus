...
import ZhihuPlusJiebaWasm from '../vendor/jieba-rs-wasm-glue.js';
import { state } from './state';

export const JIEBA_WASM_URL = 'https://cdn.jsdelivr.net/npm/jieba-wasm@2.4.0/pkg/web/jieba_rs_wasm_bg.wasm';
export let jiebaReady = false;
export let jiebaUnavailable = false;
export let jiebaPromise = null;
export const jiebaTokenCache = new Map();
export const jiebaSetCache = new Map();
export const jiebaDictSeen = new Set();
...
export function gmFetchBuffer(url) {
...
export function cutNoiseTokens(text) {
...
export function noiseTokenSet(text) {
...
export function jiebaTagTokens(text) {
...
export function isJiebaStopword(word) {
...
export function canLearnJiebaToken(word, tag) {
    if (isJiebaStopword(word)) return false;
    if (tag && !JIEBA_LEARN_POS.has(tag)) return false;
    return true;
}

export function noiseHasFallback(text, term) {
...
export function noiseHas(text, term, tokenSet) {
...
export function noiseWords(weight, list) {
